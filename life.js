...
(function () {
  ...
  var GOL = {
    ...
    // Initial state
    initialState : '[{"9":[10]},{"10":[12]},{"11":[9,10,13,14,15]}]',
    ...
    init : function() {
      try {
        ...
        this.biaobiao = new Image();
        this.biaobiao.src = "biao.png";
    ...
    loadConfig : function() {
      ...
      // Initial grid config
      grid = parseInt(this.helpers.getUrlParameter('grid'), 2);
      if (isNaN(grid) || grid < 1 || grid > GOL.grid.schemes.length) {
        grid = 1;
      }

      // Initial zoom config
      zoom = parseInt(this.helpers.getUrlParameter('zoom'), 1);
      ...
      drawCell : function (i, j, alive) {
        ...
        if (alive) {
          ...
          if (this.age[i][j] > -1)
            ...
            this.context.drawImage(GOL.biaobiao, this.cellSpace + (this.cellSpace * i) + (this.cellSize * i), this.cellSpace + (this.cellSpace * j) + (this.cellSize * j), this.cellSize, this.cellSize);        
